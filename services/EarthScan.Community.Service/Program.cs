// ---------------------------------------------------------------------------
// EarthScan Community microservice.
//
...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "EarthScan Community Service", Version = "v1" });
...
// --- Database (this service owns the 'earthscan_community' schema) --------------------
...
app.MapControllers();
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "community" }));
...
async Task MigrateWithRetryAsync(WebApplication application, int maxAttempts, TimeSpan delay)
{
    for (var attempt = 1; attempt <= maxAttempts; attempt++)
    {
        try
        {
            ...
            application.Logger.LogInformation("[community] Database ready.");
            return;
        }
        catch (Exception ex)
        {
            application.Logger.LogWarning(
                "[community] Database not ready (attempt {Attempt}/{MaxAttempts}): {Message}",
                attempt, maxAttempts, ex.Message);

            if (attempt == maxAttempts)
            {
                application.Logger.LogError(ex, "[community] Giving up on database initialisation.");
                return;
            }
