...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "EarthScan Land Service", Version = "v1" });
...
// --- Database (this service owns the 'earthscan_land' schema) --------------------
var connectionString = builder.Configuration.GetConnectionString("DefaultConnection");
builder.Services.AddDbContext<EarthScanDbContext>(options =>
    options.UseMySql(connectionString, new MySqlServerVersion(new Version(8, 0, 36))));

builder.Services.AddHttpClient<EarthScan.Backend.Services.GovernmentSatbaraService>();
...
app.MapControllers();
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "land" }));
...
async Task MigrateWithRetryAsync(WebApplication application, int maxAttempts, TimeSpan delay)
{
    for (var attempt = 1; attempt <= maxAttempts; attempt++)
    {
        try
        {
            ...
            application.Logger.LogInformation("[land] Database ready.");
            return;
        }
        catch (Exception ex)
        {
            application.Logger.LogWarning(
                "[land] Database not ready (attempt {Attempt}/{MaxAttempts}): {Message}",
                attempt, maxAttempts, ex.Message);

            if (attempt == maxAttempts)
            {
                application.Logger.LogError(ex, "[land] Giving up on database initialisation.");
                return;
            }
